...
namespace Persistance.Implementations.Services
{
    public class ReviewService:IReviewService
    {
        ...
        public async Task<GenericResponseModel<List<ReviewGetDTO>>> GetAllReviewsByPropertyId(int properyId)
        {
            ...
            var data = await _reviewRepo.GetAll().Where(x => x.PropertyId == properyId).ToListAsync();
            if (data.Count == 0)
            {
                responseModel.StatusCode = 404;
                return responseModel;
